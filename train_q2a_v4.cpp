// train_q2a_v4.cpp
// Q2-A: prove it works on a Q2-A-friendly task.
//
// Task: predict the LAST token (label = tokens[SEQ_LEN-1]).
//   - Q2-A is good at "remembering recent" because EMA weights recent more.
//   - With alpha ~0.5-0.9, last token dominates h_t.
//   - Linear head just learns to read h.
//
// If Q2-A works here but failed at majority, the issue was TASK not training.
//
// Compile: clang++ -O2 -std=c++17 -march=native -o train_q2a_v4.exe train_q2a_v4.cpp
...
const float LR_ALPHA = 0.05f;
const float ALPHA_INIT = 0.7f;  // medium - balance recent vs history
...
int main() {
    std::cout << "================================================================\n";
    std::cout << "  Q2-A v4: predict LAST token (Q2-A-friendly task)\n";
    std::cout << "================================================================\n\n";

    std::cout << "  Task: 16-token random sequence; label = tokens[15]\n";
    std::cout << "  Why this works for Q2-A:\n";
    std::cout << "    EMA weights recent tokens heavily -> h_15 reflects x_15\n";
    std::cout << "    alpha=0.7 init: weight ratio position 0/15 = 0.7^15 = 0.005 (recent wins)\n";
    ...
    for (int i = 0; i < N_TRAIN; ++i) {
        ...
        int label = seq[SEQ_LEN - 1];  // LAST token determines label
    ...
    std::cout << "  Learned alpha for 3 informative dims:\n";
    std::cout << "    alpha[0]   = " << std::setprecision(3) << alpha[0]
              << "  (sees +3 if last token is A, -3 if last is B)\n";
    std::cout << "    alpha[1]   = " << alpha[1] << "  (sees -3 if last A, +3 if last B)\n";
    std::cout << "    alpha[2]   = " << alpha[2] << "  (sees +1 if last A, -1 if last B)\n";

    std::cout << "\n  Learned w for 3 informative dims:\n";
