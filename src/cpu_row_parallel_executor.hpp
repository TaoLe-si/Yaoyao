...
public:
    ...
    CpuRowParallelExecutor():worker_([this]{loop();}) {}
    CpuRowParallelExecutor(const CpuRowParallelExecutor&)=delete;
    CpuRowParallelExecutor& operator=(const CpuRowParallelExecutor&)=delete;
    ~CpuRowParallelExecutor() {
    ...
    template<class F> void run(size_t rows,size_t cols,F fn) {
        ...
        try {fn(0,split);} catch (...) {caller_error=std::current_exception();}
        std::unique_lock<std::mutex> lock(mutex_);
