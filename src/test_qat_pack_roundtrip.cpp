...
void pack_roundtrip(W&w){size_t symbols=0,rows=0;auto row=[&](int start,int n){double scale=0;for(int i=0;i<n;++i){double v=w[start+i];if(!std::isfinite(v))throw std::runtime_error("nonfinite");if(v!=0){if(scale&&std::abs(v)!=scale)throw std::runtime_error("not ternary");scale=std::abs(v);}}if(!scale)scale=1;float alpha=float(scale);if(!std::isfinite(alpha)||alpha<=0)throw std::runtime_error("scale");std::vector<unsigned char>packed((n+3)/4,0);for(int i=0;i<n;++i){int code=w[start+i]==0?0:(w[start+i]>0?1:2);packed[i/4]|=code<<(2*(i%4));}for(int i=0;i<n;++i){int code=(packed[i/4]>>(2*(i%4)))&3;if(code==3)throw std::runtime_error("invalid code");w[start+i]=code==0?0:(code==1?double(alpha):-double(alpha));}w[start+n]=double(float(w[start+n]));symbols+=n;++rows;};for(int j=0;j<16;++j)row(j*57,56);for(int j=0;j<9;++j)row(912+j*17,16);for(int j=0;j<32;++j)row(1065+j*17,16);for(int j=0;j<16;++j)row(1609+j*33,32);if(symbols!=2064||rows!=73)throw std::runtime_error("layout");}
int main(){try{for(unsigned seed:{7u,123u,31415u})for(int shift:{0,1,2,3}){split_shift=shift;Net net(seed);std::ifstream f("build/qat_spaced_0_91357_1000_partition_"+std::to_string(shift)+"_"+std::to_string(seed)+".weights",std::ios::binary);if(!f.read((char*)net.w.data(),sizeof(net.w))||f.peek()!=EOF)throw std::runtime_error("weights");Net original=net;pack_roundtrip(net.w);for(double threshold:{0.}){tao::ResidualWriteThreshold<32> m(net.w);int correct=0,updated=0,parity=0;for(int syntax=0;syntax<3;++syntax){std::mt19937 rng(583921+syntax);for(int i=0;i<64;++i){auto base=heldout(rng,128);auto other=base;other.target=(base.target+4)%8;for(size_t j=other.tokens.size();j>0;j-=3)if(other.tokens[j-3]==base.query){other.tokens[j-2]=4+other.target;break;}auto layout=rng;auto s=grammar(base,rng,syntax),changed=grammar(other,layout,syntax);m.reset();for(int t:s.tokens)m.step(s.query,t,threshold);correct+=m.memory==s.target;if(threshold==0){int ref;net.run(s,nullptr,nullptr,&ref);if(ref!=m.memory)throw std::runtime_error("decoded parity");original.run(s,nullptr,nullptr,&ref);if(ref!=m.memory)throw std::runtime_error("original parity");++parity;}m.reset();for(int t:changed.tokens)m.step(changed.query,t,threshold);updated+=m.memory==changed.target;}}printf("seed=%u shift=%d threshold=%+.1f correct=%d/192 update=%d/192 parity=%d\n",seed,shift,threshold,correct,updated,parity);}}return 0;}catch(const std::exception&e){printf("FAIL %s\n",e.what());return 1;}}
