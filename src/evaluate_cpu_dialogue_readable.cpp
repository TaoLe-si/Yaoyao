...
for(size_t turn=0;turn<sessions[sid].size();++turn){if(closed){printf("SKIP session ended by EOS\n");break;}auto prompt=sessions[sid][turn];printf(u8"用户：%s\n",prompt.c_str());auto start=std::chrono::steady_clock::now();logits=m.step(257,state);for(auto id:b.encode(prompt))logits=m.step(id,state);logits=m.step(259,state);logits=m.step(258,state);auto decode_start=std::chrono::steady_clock::now();std::vector<uint32_t> ids;int end=-1;
