...
for(int j=0;
j<batch;
++j){int noise=data_rng()%3;
auto fresh=grammar(training(data_rng,step<=500?0:noise),data_rng,int(data_rng()%3));
...
}std::ofstream out("build/curriculum_factor4_"+std::to_string(seed)+".weights",std::ios::binary);
