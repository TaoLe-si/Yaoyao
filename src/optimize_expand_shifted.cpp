...
t<s.tokens.size();
t+=3)if((s.tokens[t+1]-4)%4==(s.tokens[t]+1)%4)return false;
return true;
}
Seq training(std::mt19937&r,int noise){auto s=make(r,noise);
for(size_t t=0;
t<s.tokens.size();
t+=3)while((s.tokens[t+1]-4)%4==(s.tokens[t]+1)%4)s.tokens[t+1]=4+r()%8;
...
}std::ofstream out("build/expand_shifted_write4_"+std::to_string(seed)+".weights",std::ios::binary);
