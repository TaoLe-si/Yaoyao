...
#include "advance_row_parallel_avx512_cpu_model.hpp"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
int main(int argc,char**argv){try{
 ...
 if(argc>3)throw std::runtime_error("usage: benchmark_advance_row_avx512 [bundle [tokenizer_hash]]");
 ...
 const AdvanceRowParallelAvx512CpuModel candidate(read_compact_bundle(path,hash));
 ...
 Trial base[2],cand[2];
 base[0]=bench(baseline,"compact_avx2_2thread");cand[0]=bench(candidate,"fixed_avx512_2thread");
 cand[1]=bench(candidate,"fixed_avx512_2thread");base[1]=bench(baseline,"compact_avx2_2thread");
