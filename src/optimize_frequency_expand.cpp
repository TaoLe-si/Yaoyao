...
int exposure=0;std::mt19937 injection(82419);unsigned long long counts[4][8]={};
Seq exposed(std::mt19937&r,int noise){auto s=training(r,noise);for(size_t j=0;j<s.tokens.size();j+=3){if(int(injection()%10000)<exposure)s.tokens[j+1]=4+(s.tokens[j]+split_shift)%4+4*(injection()%2);++counts[s.tokens[j]][s.tokens[j+1]-4];}for(size_t j=s.tokens.size();j>0;j-=3)if(s.tokens[j-3]==s.query){s.target=s.tokens[j-2]-4;break;}return s;}
...
}for(auto&row:counts)for(auto&c:row)c=0;
W gradient{},m{},v{};
...
std::ofstream out("build/frequency_expand_"+std::to_string(exposure)+"_partition_"+std::to_string(split_shift)+"_"+std::to_string(seed)+".weights",std::ios::binary);
