...
#include "diagnose_smooth_factor4.cpp"
#undef main
#include <set>
#include <sstream>
#include <fstream>
...
auto base=training(data_rng,data_rng()%3);
auto changed=base;
for(size_t k=0;k<changed.tokens.size();k+=3)if(changed.tokens[k]!=changed.query){
int who;do{who=int(data_rng()%8);}while(who%4==changed.tokens[k]||who==changed.tokens[k+1]-4);
changed.tokens[k+1]=4+who;
}
if(!allowed(changed)||changed.target!=base.target)throw std::runtime_error("augmentation split");
int syntax=data_rng()%3;
auto layout=data_rng;
auto fresh=grammar(base,data_rng,syntax);
auto paired=grammar(changed,layout,syntax);
seen.insert(key(fresh));seen.insert(key(paired));
net.run(fresh,&gradient);net.run(paired,&gradient);
}for(int i=0;
i<N;
++i){double g=gradient[i]/batch;
...
}std::ofstream out("build/smooth_nol2_factor4_"+std::to_string(seed)+".weights",std::ios::binary);
