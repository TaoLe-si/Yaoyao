// Independent model snapshot: only dot kernel changed; CpuModel input validated.
#pragma once
#include "dual_state_cpu.hpp"
#include "cpu_ternary_signmask_avx2.hpp"
...
struct SignMaskByteCpuModel{
Config c;std::map<std::string,Vec>w;std::map<std::string,CpuTernarySignMaskRows>packed;
explicit SignMaskByteCpuModel(const CpuModel&src):c(src.c){for(auto&t:schema(c)){if(t.ternary)packed.emplace(t.name,CpuTernarySignMaskRows(src.w.at(t.name),t.rows,t.cols));else w.emplace(t.name,src.w.at(t.name));}}
#ifdef TAO_CPU_AVX2
bool fast=true;
// Opt-in experimental backend; existing callers retain the single-row kernel.
// Single-row sign-mask candidate only.
#endif
