...
for(int step=1;
step<=steps;
++step){train_step=step;gradient.fill(0);
...
std::ofstream out("build/role_spaced_"+std::to_string(role_mode)+"_"+std::to_string(data_seed)+"_"+std::to_string(exposure)+"_partition_"+std::to_string(split_shift)+"_"+std::to_string(seed)+".weights",std::ios::binary);
