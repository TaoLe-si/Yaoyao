...
ReusableBatchGraph(SortedGpuTrainer&tr,unsigned slots,unsigned width,bool grpo=false,float beta=0.f,float eps=0.2f,float* dbg=nullptr):batch(tr.graph.c,slots,tr.graph.w),data(blank(slots,width),tr.graph.c.vocab){data.grpo=grpo;data.beta=beta;data.eps=eps;data.dbg=dbg;
...
copy(*data.inputs,*upload.inputs);copy(*data.active,*upload.active);copy(*data.reset,*upload.reset);copy(data.targets,upload.targets);copy(data.masks,upload.masks);copy(data.weights,upload.weights);
// 【必须拷贝】lpold/lpref 是论文 Eq.3 的关键输入：缺 lpold 则 r=exp(lp_theta-0)=pi_theta，
// 裁剪项彻底失真；缺 lpref 则 KL 的 d=0、梯度恒为 0，KL 静默失效。
// 早期版本漏了这两行，属于「参数传了但没送到内核」的静默错误，故在此显式标注。
copy(data.lpold,upload.lpold);copy(data.lpref,upload.lpref);
