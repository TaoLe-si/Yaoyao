...
#include "packed_2bit_lut_cpu_model.hpp"
#include <chrono>
#include <cstdio>
#include <cstring>
#include <algorithm>
...
static void synthetic() {
    ...
    for(size_t c:widths) {
        ...
        CpuTernaryRows byte(w,rows,c); CpuTernary2BitLutRows packed(w,rows,c);
    ...
    bool rejected=false;
    try { CpuTernary2BitLutRows bad(std::vector<float>{1,2},1,2); }
    ...
    CpuTernaryRows byte(w,rows,cols); CpuTernary2BitLutRows packed(w,rows,cols);
    ...
    for(int trial=0;trial<4;++trial) {
        ...
        std::printf("DOT trial=%d byte_seconds=%.6f lut_seconds=%.6f byte_over_packed=%.4f byte_weight_bytes=%zu packed_weight_bytes=%zu\n",
...
int main(int argc,char** argv) {
    try {
        synthetic();
        if(argc==1) {std::puts("Full-model optional: benchmark_ternary_2bit_lut <bundle.dsb> <tokenizer_sha256> [steps=128]");return 0;}
        ...
        ByteCpuModel byte(src); Packed2BitLutCpuModel packed(src);
        ...
        for(int trial=0;trial<4;++trial) {
            ...
            std::printf("FULL_GREEDY trial=%d forced_steps=%d byte_tps=%.3f lut_tps=%.3f packed_over_byte=%.4f\n",trial,steps,bt,pt,pt/bt);
        }
        return 0;
