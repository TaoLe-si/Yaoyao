...
#include "diagnose_hardgate_expand.cpp"
#undef main
#include <set>
#include <sstream>
#include <fstream>
...
int train_step=0;int role_mode=0;int exposure=0;std::mt19937 injection(82419);unsigned long long counts[4][8]={},roles[3][4][8]={};
Seq exposed(std::mt19937&r,int noise){auto s=training(r,noise);size_t last=0;std::vector<size_t>irrelevant;for(size_t j=0;j<s.tokens.size();j+=3){if(s.tokens[j]==s.query)last=j;else irrelevant.push_back(j);}if(int(injection()%10000)<exposure){if(role_mode==0||!irrelevant.empty()){size_t j=role_mode==0?last:irrelevant[injection()%irrelevant.size()];int owner=(s.tokens[j]+split_shift)%4+4*(injection()%2);if(counts[s.tokens[j]][owner]<(train_step==0?64:std::min(64,1+(train_step-1)*64/2800)))s.tokens[j+1]=4+owner;}}s.target=s.tokens[last+1]-4;for(size_t j=0;j<s.tokens.size();j+=3){int role=j==last?0:(s.tokens[j]==s.query?1:2);++counts[s.tokens[j]][s.tokens[j+1]-4];++roles[role][s.tokens[j]][s.tokens[j+1]-4];}return s;}
void ternary_row(W&w,int start,int n){std::vector<double>a;for(int i=0;i<n;++i)a.push_back(std::abs(w[start+i]));std::sort(a.begin(),a.end(),std::greater<double>());double sum=0,best=-1,scale=0;int count=0;for(int k=1;k<=n;++k){sum+=a[k-1];double score=sum*sum/k;if(score>best){best=score;scale=sum/k;count=k;}}double cutoff=a[count-1];for(int i=0;i<n;++i){double v=w[start+i];w[start+i]=std::abs(v)>=cutoff?(v<0?-scale:scale):0;}}
void quantize(W&w,int mode){if(mode==1||mode==4)for(int j=0;j<16;++j)ternary_row(w,j*57,56);if(mode==2||mode==4){for(int j=0;j<32;++j)ternary_row(w,1065+j*17,16);for(int j=0;j<16;++j)ternary_row(w,1609+j*33,32);}if(mode==3||mode==4)for(int j=0;j<9;++j)ternary_row(w,912+j*17,16);}
...
for(int step=1;
step<=steps;
++step){train_step=step;gradient.fill(0);W master=net.w;quantize(net.w,4);
...
for(int object=0;object<4;++object)for(int owner=0;owner<8;++owner)if(owner%4==(object+split_shift)%4&&counts[object][owner]!=64)throw std::runtime_error("quota unmet");
...
std::ofstream out("build/qat_hardgate_"+std::to_string(role_mode)+"_"+std::to_string(data_seed)+"_"+std::to_string(exposure)+"_partition_"+std::to_string(split_shift)+"_"+std::to_string(seed)+".weights",std::ios::binary);
