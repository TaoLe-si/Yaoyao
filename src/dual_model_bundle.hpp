...
#include <sstream>
#include <filesystem>
#if defined(TAO_NO_FFN)&&defined(TAO_DELTA_MEM)
#define TAO_OPERATOR_ID "dual-state-4-noffn-delta-mem-input-sqrt-d"
#elif defined(TAO_NO_FFN)
...
inline uint64_t model_bytes(const Config&c){uint64_t n=28;
#ifdef TAO_DELTA_MEM
n+=4;
#endif
for(auto&t:schema(c))n+=t.ternary?uint64_t(t.rows)*(4+(uint64_t(t.cols)+3)/4):4*t.elements();return n;}
...
{auto nl=bytes.find('\n',28);if(nl==std::string::npos||nl<=28||bytes.compare(28,nl-28,TAO_OPERATOR_ID)!=0)throw std::runtime_error("operator identity mismatch");}auto get=[&](size_t pos){uint64_t v=0;for(int i=0;i<8;++i)v|=uint64_t((unsigned char)bytes[pos+i])<<(8*i);return v;};auto mn=get(4),pn=get(12);if(mn>1024*1024||pn<28||mn>bytes.size()-28||pn!=bytes.size()-28-mn)throw std::runtime_error("bundle lengths");if(bundle_hash(bytes.substr(28))!=get(20))throw std::runtime_error("checksum");auto payload=bytes.substr(28+mn);auto u=[&](size_t off){uint32_t v=0;for(int i=0;i<4;++i)v|=uint32_t((unsigned char)payload[off+i])<<(8*i);return v;};Config c;c.layers=u(4);c.d=u(8);c.s=u(12);c.m=u(16);c.e=u(20);c.vocab=u(24);
#ifdef TAO_DELTA_MEM
c.dk=u(28);
#endif
