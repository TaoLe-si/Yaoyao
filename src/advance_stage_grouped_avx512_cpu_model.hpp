...
#include "cpu_ternary_avx512_fixed.hpp"
...
// Standalone grouped-stage + fixed AVX512 owning candidate.
// Caller must guard AVX2/AVX512F/BW/DQ + OS XCR0 before use.
// Build caller TU AVX2, fixed kernel TU AVX512, strict FP and no LTO/LTCG.
// Exactly one caller plus one persistent sleeping worker for all projections.
struct AdvanceStageGroupedAvx512CpuModel{
Config c;std::map<std::string,Vec>w;std::map<std::string,CpuTernaryRows512Fixed>packed;
explicit AdvanceStageGroupedAvx512CpuModel(CompactBundleData&&src):c(src.c),w(std::move(src.vectors)){for(auto&t:schema(c))if(t.ternary){auto&m=src.matrices.at(t.name);CpuTernaryRows512Fixed p(Vec(t.cols,0),1,t.cols);p.rows=t.rows;p.q=std::move(m.q);p.scale=std::move(m.scale);packed.emplace(t.name,std::move(p));}}
explicit AdvanceStageGroupedAvx512CpuModel(const CpuModel&src):c(src.c){for(auto&t:schema(c)){if(t.ternary)packed.emplace(t.name,CpuTernaryRows512Fixed(src.w.at(t.name),t.rows,t.cols));else w.emplace(t.name,src.w.at(t.name));}}
#ifdef TAO_CPU_AVX2
static constexpr bool fast=true;
// Opt-in experimental backend; existing callers retain the single-row kernel.
// Fixed split-TU AVX512 dot preserves the original eight-lane accumulation order.
...
template<size_t N> std::array<Vec,N> group(const std::array<std::string,N>& names,
                                         ...
                                         uint32_t rows)const {
    std::array<Vec,N> out;
    std::array<const CpuTernaryRows512Fixed*,N> matrices{};
