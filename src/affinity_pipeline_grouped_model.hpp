#pragma once
#include "cpu_affinity_experiment.hpp"
...
struct AffinityPipelineGroupedModel{
Config c;std::map<std::string,Vec>w;std::map<std::string,PipelineRows>packed;
explicit AffinityPipelineGroupedModel(CompactBundleData&&src):c(src.c),w(std::move(src.vectors)){for(auto&t:schema(c))if(t.ternary){auto&m=src.matrices.at(t.name);PipelineRows p(Vec(t.cols,0),1,t.cols);p.rows=t.rows;p.q=std::move(m.q);p.scale=std::move(m.scale);packed.emplace(t.name,std::move(p));}}
explicit AffinityPipelineGroupedModel(const CpuModel&src):c(src.c){for(auto&t:schema(c)){if(t.ternary)packed.emplace(t.name,PipelineRows(src.w.at(t.name),t.rows,t.cols));else w.emplace(t.name,src.w.at(t.name));}}
#ifdef TAO_CPU_AVX2
bool fast=true;
...
public:
    void enable_affinity(unsigned caller=0,unsigned worker=2)const {executor.enable(caller,worker);}
    void disable_affinity()const {executor.disable();}
...
mutable CpuAffinityExperimentExecutor executor;
};
}
