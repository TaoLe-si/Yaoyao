...
#define DIAGNOSTIC_PREFIX "stop_probe_step_"
#endif
...
std::string control_path="build/batch_stop_probe.control";
...
if(!preflight&&(control.stop||std::filesystem::exists("build/STOP_BATCH_PROBE"))){printf("STOP restored checkpoint retained; no updates\n");return 0;}
...
if(std::filesystem::exists("build/STOP_BATCH_PROBE")){save();printf("STOP optimizer boundary\n");break;}
printf("PROBE_UPDATE_BEGIN\n");fflush(stdout);auto update_start=std::chrono::steady_clock::now();
