...
auto paired=fresh;for(auto&token:paired.tokens)if(token>=4&&token<12)token=4+(token-4+4)%8;
paired.target=(paired.target+4)%8;
seen.insert(key(paired));net.run(paired,&gradient);
}for(int i=0;
i<N;
...
}std::ofstream out("build/paired_expand_partition_"+std::to_string(split_shift)+"_"+std::to_string(seed)+".weights",std::ios::binary);
