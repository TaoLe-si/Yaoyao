...
namespace tao::dual::diagnostic_validation {
...
struct Corpus {
    ...
    // Diagnostic ONLY: caller chooses 23 documents; hashes describe full source corpus.
    Corpus(std::vector<std::vector<tao::data::Token>> selected,
           std::vector<size_t> ids,const std::string& dataset_hash,const std::string& tokenizer_hash)
        :dataset_sha256(dataset_hash),tokenizer_sha256(tokenizer_hash),docs(std::move(selected)),doc_ids(std::move(ids)) {
        if(docs.size()!=23||doc_ids.size()!=23||dataset_sha256.size()!=64||tokenizer_sha256!=fixed_tokenizer_sha256)
            throw std::runtime_error("diagnostic corpus contract");
        for(const auto&d:docs){if(d.size()<3)throw std::runtime_error("diagnostic short doc");
            steps=(std::max)(steps,d.size()-1);tokens+=d.size();positions+=d.size()-1;
            for(size_t j=1;j<d.size();++j)supervised+=d[j].loss;}
        if(!supervised)throw std::runtime_error("diagnostic empty supervision");
    }
};
struct Result {
    Totals totals{}; unsigned step=0;
    std::vector<Totals> per_doc;
    std::string dataset_sha256,tokenizer_sha256;
    size_t documents=23,tokens=0,parameter_h2d_bytes=0,hotpath_h2d_bytes=0,d2h_bytes=24*sizeof(Totals);
    const char* backend="cuda-forward-arch2-frozen-diagnostic-v1";
...
public:
    ...
    Result run(){
        ...
        result.per_doc.resize(slots);
        check(cudaMemcpyAsync(result.per_doc.data(),doc_totals.p,slots*sizeof(Totals),cudaMemcpyDeviceToHost,0));
        check(cudaStreamSynchronize(0));
        for(size_t k=0;k<corpus.docs.size();++k){size_t expected=0;for(size_t j=1;j<corpus.docs[k].size();++j)expected+=corpus.docs[k][j].loss;
            if(result.per_doc[k].bad||!std::isfinite(result.per_doc[k].loss)||result.per_doc[k].positions!=corpus.docs[k].size()-1||result.per_doc[k].supervised!=expected)throw std::runtime_error("diagnostic per-doc totals");}
        if(result.totals.bad||!std::isfinite(result.totals.loss)||result.totals.supervised!=corpus.supervised||
