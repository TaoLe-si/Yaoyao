#pragma once
#include "cpu_configurable_row_executor.hpp"
...
struct ConfigurableRowByteCpuModel{
Config c;std::map<std::string,Vec>w;std::map<std::string,CpuTernaryRows>packed;
explicit ConfigurableRowByteCpuModel(CompactBundleData&&src,size_t threads=2,size_t threshold=262144):c(src.c),w(std::move(src.vectors)),executor(threads,threshold){for(auto&t:schema(c))if(t.ternary){auto&m=src.matrices.at(t.name);CpuTernaryRows p(Vec(t.cols,0),1,t.cols);p.rows=t.rows;p.q=std::move(m.q);p.scale=std::move(m.scale);packed.emplace(t.name,std::move(p));}}
explicit ConfigurableRowByteCpuModel(const CpuModel&src,size_t threads=2,size_t threshold=262144):c(src.c),executor(threads,threshold){for(auto&t:schema(c)){if(t.ternary)packed.emplace(t.name,CpuTernaryRows(src.w.at(t.name),t.rows,t.cols));else w.emplace(t.name,src.w.at(t.name));}}
#ifdef TAO_CPU_AVX2
bool fast=true;
...
mutable CpuConfigurableRowExecutor executor;
};
}
