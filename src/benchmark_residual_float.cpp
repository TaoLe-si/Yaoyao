...
int main(){try{for(unsigned seed:{7u,123u,31415u}){Net n(seed);std::ifstream f("build/role_spaced_0_91357_1000_partition_0_"+std::to_string(seed)+".weights",std::ios::binary);if(!f.read((char*)n.w.data(),sizeof(n.w)))throw std::runtime_error("weights");tao::ResidualWriteThreshold<32>d(n.w);tao::ResidualWriteFloat<32>x(n.w);std::mt19937 r(193);auto s=grammar(heldout(r,32),r,0);double td=measure(d,s),tf=measure(x,s);printf("seed=%u FP64_us=%.6f FP32_us=%.6f ratio=%.3f\n",seed,td,tf,tf/td);}return 0;}catch(const std::exception&e){printf("FAIL %s\n",e.what());return 1;}}
