...
#define DIAGNOSTIC_PREFIX "full_batch_step_"
#endif
...
for(int r=0;r<8;++r){auto plan=tao::data::take_batch(cursor,256);if(!plan.timesteps)continue;GpuBatchPlan data(plan,tr.graph.c.vocab);for(size_t t=0;t<plan.timesteps;++t){std::vector<unsigned>ids(4);std::vector<bool>active(4),reset(4);for(unsigned k=0;k<4;++k){auto&p=plan.items[t*4+k];ids[k]=p.input;active[k]=p.active;reset[k]=p.reset;}auto y=batch.step(ids,active,reset);data.seed(y,t);}if(plan.supervised)batch.tape.backward();accumulate_loss<<<1,1>>>(data.loss.p,plan.items.size(),block_loss.total,block_loss.bad);detach_batch(batch);n+=plan.supervised;positions+=plan.positions;}bridge_slots(batch,slots,false);
