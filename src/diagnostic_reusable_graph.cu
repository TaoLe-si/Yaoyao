...
#define DIAGNOSTIC_PREFIX "full_reusable_graph_step_"
#endif
...
auto capture_start=std::chrono::steady_clock::now();ReusableBatchGraph replay(tr,4,256);printf("GRAPH_BUILD seconds=%.6f\n",std::chrono::duration<double>(std::chrono::steady_clock::now()-capture_start).count());DeferredLoss block_loss(1024);
