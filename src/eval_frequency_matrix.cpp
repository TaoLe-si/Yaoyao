...
int main(){try{for(unsigned seed:{7u,123u,31415u})for(int shift:{0,1,2,3}){split_shift=shift;Net net(seed);std::ifstream f("build/frequency_expand_10_partition_"+std::to_string(shift)+"_"+std::to_string(seed)+".weights",std::ios::binary);if(!f.read((char*)net.w.data(),sizeof(net.w))||f.peek()!=EOF)throw std::runtime_error("weights");for(double threshold:{0.}){tao::ResidualWriteThreshold<32> m(net.w);int correct=0,updated=0,parity=0;for(int syntax=0;syntax<3;++syntax){std::mt19937 rng(671829+syntax);for(int i=0;i<64;++i){auto base=heldout(rng,128);auto other=base;other.target=(base.target+4)%8;for(size_t j=other.tokens.size();j>0;j-=3)if(other.tokens[j-3]==base.query){other.tokens[j-2]=4+other.target;break;}auto layout=rng;auto s=grammar(base,rng,syntax),changed=grammar(other,layout,syntax);m.reset();for(int t:s.tokens)m.step(s.query,t,threshold);correct+=m.memory==s.target;if(threshold==0){int ref;net.run(s,nullptr,nullptr,&ref);if(ref!=m.memory)throw std::runtime_error("parity");++parity;}m.reset();for(int t:changed.tokens)m.step(changed.query,t,threshold);updated+=m.memory==changed.target;}}printf("seed=%u shift=%d threshold=%+.1f correct=%d/192 update=%d/192 parity=%d\n",seed,shift,threshold,correct,updated,parity);}}return 0;}catch(const std::exception&e){printf("FAIL %s\n",e.what());return 1;}}
