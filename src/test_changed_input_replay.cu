...
Vec changed{81,82,83,84};check(cudaMemcpy(ids->p,changed.data(),16,cudaMemcpyHostToDevice));for(auto&kv:b.w)check(cudaMemsetAsync(kv.second->grad.p,0,kv.second->grad.n*4));check(cudaGraphLaunch(executable,cudaStreamPerThread));check(cudaStreamSynchronize(cudaStreamPerThread));BatchTrainGraph fresh(cpu,4);auto new_direct=fresh.step_device(ids,mask,mask,0);tape_copy(new_direct->grad.p,seed.p,1044*4);fresh.tape.backward();check(cudaDeviceSynchronize());
...
size_t nodes=0;check(cudaGraphGetNodes(graph,nullptr,&nodes));printf("CHANGED_INPUT_REPLAY nodes=%zu maxabs=%.9g launches=2\n",nodes,max);check(cudaGraphExecDestroy(executable));check(cudaGraphDestroy(graph));return max==0?0:1;
