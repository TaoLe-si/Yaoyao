// Continue expanded-data repair from a verified shuffled-native checkpoint.
// Does not overwrite production or the source checkpoint directory.
#define main preserved_continue_production_main
...
const std::string policy="expanded-continue-v1;resume-verified-shuffled-native;preserve-master-adam-step-cursor-states;no-fresh-reset;canonical-BPE-no-legacy-EOS-removal;splitmix64-fisher-yates-v1;seed=20260909;graph=4x8x256;lr=.00025;official-gpuval-every-update;max200";
...
int main(int argc,char** argv){try{
    using namespace continued;
    require(argc==8,"usage: train_repair_continue SOURCE_CKPT_DIR TRUSTED_MANIFEST_SHA256 NEW_TRAIN_BIN PARENT_RECEIPT TRUSTED_RECEIPT_SHA256 UNIQUE_NEW_CHECKPOINT_DIR UPDATES_1_TO_200");
    stopped();disk::hash_self_test();
    std::string count=argv[7];require(!count.empty()&&count.size()<=3&&count.find_first_not_of("0123456789")==std::string::npos,"updates syntax");
    ...
    require(fs::is_directory(fs::symlink_status(source)),"source checkpoint directory missing");
    require(disk::digest(argv[2]),"trusted source manifest digest");
    ...
    require(disk::digest(argv[5])&&disk::sha256(receipt)==argv[5],"parent receipt digest must be supplied independently by parent");
    ...
    require(field("dataset_sha256")==newsha&&field("tokenizer_sha256")==th&&field("manifest_sha256")==completion_sha
        &&!receipt.empty()&&receipt.back()=='\n'&&accepted.peek()==std::char_traits<char>::eof(),"parent receipt mismatch");
    ...
    DeterministicShuffledEpochCursor epoch(std::move(canonical),seed,verified);
    shuffled_native::load_and_restore(source,argv[2],slots,epoch,seed,verified,identity);
    require(tr.steps==1240 && epoch.epoch()==1,"resume must restore step 1240 epoch 1");
    const auto continue_identity=checkpoint_identity(tr.graph.c,newsha,th,200)+policy+"\nsource_manifest_sha256="+std::string(argv[2])+"\nparent_receipt_sha256="+argv[5]+"\n";
    const auto continue_verified=continue_identity+"canonical-new-dataset-sha256="+newsha+"\nresumed-from-step=1240\n";
    ...
    require(std::abs(initial.nll()-6.3094786034531669)<1e-12,"resumed validation NLL does not match completed 1240 diagnostic");
    metrics<<"RESUME step="<<tr.steps<<" positions=0 targets=0 train_preupdate_NLL=NA val_NLL="<<initial.nll()<<" dataset_positions="<<dataset_positions<<" dataset_targets="<<dataset_targets<<"\n";
    ...
    for(unsigned u=0;u<updates;++u){
        ...
        std::ostringstream uline;uline.precision(17);uline<<"UPDATE step="<<tr.steps<<" epoch="<<epoch.epoch()<<" positions="<<positions<<" targets="<<targets<<" train_preupdate_NLL="<<train_loss/targets<<" val_NLL="<<v.nll()<<" lr=.00025 norm="<<norm<<"\n";
        metrics<<uline.str();printf("%s",uline.str().c_str());fflush(stdout);
    }
    stopped();require(tr.steps==start+updates,"bounded final step");
    const auto commit=shuffled_native::save(out,slots,epoch,seed,continue_verified,continue_identity);
    ...
    disk::publish_text(out/"transition.identity",continue_identity);
    disk::publish_text(out/"verified.identity",continue_verified);
    ...
    return 0;
}catch(const std::exception& e){fprintf(stderr,"CONTINUE_PILOT_FAIL %s; no automatic retry/resume; retain STOP_TRAINING\n",e.what());return 1;}}
