// Parallel TLP2 export (multi-core BPE). Never trains.
// Usage: export_noffn_probe_par conversations.txt train.bin tokenizer.bbp
//
// Output is byte-identical to the serial exporter; only the BPE phase is
// parallelised. Two costs dominate the serial path and both are removed here:
//   1. ByteBpe::validate() rebuilt a 16,123-node std::map on EVERY encode()
//      call. It is now validated once up front (cache in byte_bpe.hpp).
//   2. encode() ran on one thread. Messages are independent, so they are
//      encoded by a worker pool and then serialised in the original order.
#define NOMINMAX
...
static unsigned thread_count() {
    const char* env=std::getenv("TAO_EXPORT_THREADS");
    if (env) { int v=std::atoi(env); if (v>0) return unsigned(v); }
...
int main(int argc, char** argv) {
    try {
        require(argc==4, "usage: export_noffn_probe_par CONVERSATIONS.txt TRAIN.bin TOKENIZER.bbp");
        ...
        b.validate();  // once, not per message
        ...
        // Flatten every message, keeping its position so output order is unchanged.
        struct Ref { uint32_t doc; uint32_t idx; };
        std::vector<Ref> refs; std::vector<const std::string*> texts;
        for (size_t d=0; d<docs.size(); ++d)
            for (size_t m=0; m<docs[d].size(); ++m) {
                refs.push_back({uint32_t(d), uint32_t(m)});
                texts.push_back(&docs[d][m].utf8);
            }
        std::vector<std::vector<uint32_t>> enc(refs.size());
        const unsigned nthreads=thread_count();
        std::cerr<<"EXPORT_PARALLEL threads="<<nthreads<<" messages="<<refs.size()<<"\n";
        {
            std::atomic<size_t> next{0};
            std::vector<std::thread> pool; pool.reserve(nthreads);
            std::exception_ptr err=nullptr; std::atomic<bool> failed{false};
            for (unsigned t=0; t<nthreads; ++t)
                pool.emplace_back([&]{
                    try {
                        for (;;) {
                            size_t i=next.fetch_add(1);
                            if (i>=refs.size()||failed.load()) break;
                            auto ids=b.encode(*texts[i]);
                            require(b.decode(ids)==*texts[i], "BPE roundtrip");
        ...
        for (size_t d=0;d<docs.size();++d) {
            ...
            size_t bytes=0;
            for (size_t m=0;m<docs[d].size();++m,++flat) {
                const auto& msg=docs[d][m];
                require(refs[flat].doc==d && refs[flat].idx==m, "flatten order");
                tokens.push_back({msg.assistant?ASSISTANT:USER,false});
                for (auto id: enc[flat]) tokens.push_back({int(id), msg.assistant});
                tokens.push_back({TURN_END, msg.assistant});
                bytes+=msg.utf8.size();
            }
