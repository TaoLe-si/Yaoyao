...
allocation_seconds=free_seconds=0;allocation_calls=free_calls=0;
...
if(profile&&local){check(cudaDeviceSynchronize());double seconds=std::chrono::duration<double>(std::chrono::steady_clock::now()-block_start).count();printf("PROFILE_BLOCK positions=%zu supervised=%zu seconds=%.6f positions_per_second=%.3f no_update_no_save\n",work.end-work.begin,local,seconds,(work.end-work.begin)/seconds);printf("ALLOC calls=%zu seconds=%.6f FREE calls=%zu seconds=%.6f\n",allocation_calls,allocation_seconds,free_calls,free_seconds);printf("POOL hits=%zu misses=%zu cached_bytes=%zu\n",device_pool().hits,device_pool().misses,device_pool().cached);printf("PROFILE_NLL %.12f\n",loss/n);std::ofstream dump(
#ifdef TAO_WARP_MATVEC
"build/pool_warp_profile_grad.bin"
#else
"build/pool_warp_profile_grad.bin"
#endif
