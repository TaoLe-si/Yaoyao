...
check(cudaDeviceSynchronize());auto fwd_end=std::chrono::steady_clock::now();
loss+=block_loss.collect();auto loss_end=std::chrono::steady_clock::now();
if(local)tr.graph.tape.backward();auto bwd_end=std::chrono::steady_clock::now();
tr.detach();slots.s[slots.active]=tr.graph.s;slots.m[slots.active]=tr.graph.m;slots.active=-1;check(cudaDeviceSynchronize());auto teardown_end=std::chrono::steady_clock::now();
printf("PHASE_SECONDS forward=%.6f loss_transfer=%.6f backward=%.6f detach_teardown=%.6f sync_included=1\n",std::chrono::duration<double>(fwd_end-block_start).count(),std::chrono::duration<double>(loss_end-fwd_end).count(),std::chrono::duration<double>(bwd_end-loss_end).count(),std::chrono::duration<double>(teardown_end-bwd_end).count());n+=local;
...
#else
"build/reference_block_grad.bin"
#endif
