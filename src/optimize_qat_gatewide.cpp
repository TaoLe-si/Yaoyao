...
void quantize(W&w,int mode){if(mode==1||mode==4)for(int j=0;j<16;++j)ternary_row(w,j*57,56);if(mode==2||mode==4){for(int j=0;j<32;++j)ternary_row(w,1065+j*17,16);for(int j=0;j<16;++j)ternary_row(w,1609+j*33,32);}if(mode==3||mode==4)for(int j=1;j<9;++j)ternary_row(w,912+j*17,16);}
...
std::ofstream out("build/qat_gatewide_"+std::to_string(role_mode)+"_"+std::to_string(data_seed)+"_"+std::to_string(exposure)+"_partition_"+std::to_string(split_shift)+"_"+std::to_string(seed)+".weights",std::ios::binary);
