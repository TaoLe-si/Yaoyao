...
#define DIAGNOSTIC_PREFIX "batch_memory_step_"
#endif
...
control.target=tr.steps+1;control.stop=false;target_step=control.target;
...
size_t n=0,positions=0;double loss=0;size_t min_free=SIZE_MAX,total_mem=0;auto sample=[&](int round,const char*phase){size_t free;check(cudaMemGetInfo(&free,&total_mem));min_free=std::min(min_free,free);printf("MEM_SAMPLE round=%d phase=%s free=%zu total=%zu\n",round,phase,free,total_mem);};
for(int r=0;r<8;++r){auto plan=tao::data::take_batch(cursor,256);if(!plan.timesteps)continue;GpuBatchPlan data(plan,tr.graph.c.vocab);for(size_t t=0;t<plan.timesteps;++t){auto y=batch.step_device(data.inputs,data.active,data.reset,t*plan.slots);data.seed(y,t);}sample(r,"forward_graph_live");if(plan.supervised)batch.tape.backward();sample(r,"after_backward_before_detach");accumulate_loss<<<1,1>>>(data.loss.p,plan.items.size(),block_loss.total,block_loss.bad);detach_batch(batch);n+=plan.supervised;positions+=plan.positions;}bridge_slots(batch,slots,false);
if(!n){printf("NO_UPDATE no supervised targets\n");continue;}
loss=block_loss.collect();
unsigned step=tr.steps+1;
float lr=control.lr;
float norm=tr.update(n,lr);printf("MEM sampled_min_free=%zu sampled_max_used=%zu not_exact_peak=1\n",min_free,total_mem-min_free);
