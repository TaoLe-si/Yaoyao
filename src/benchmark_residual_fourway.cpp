...
int main(){try{for(unsigned seed:{7u,123u,31415u}){Net n(seed);std::ifstream f("build/qat_spaced_0_91357_1000_partition_0_"+std::to_string(seed)+".weights",std::ios::binary);if(!f.read((char*)n.w.data(),sizeof(n.w)))throw std::runtime_error("weights");tao::ResidualWriteThreshold<32>d(n.w);PackedResidual x(n.w);std::mt19937 r(193);auto s=grammar(heldout(r,32),r,0);PackedResidualLut lut(n.w);tao::ResidualWriteFloat<32>fp(n.w);double totals[4]={};for(int round=0;round<4;++round)for(int k=0;k<4;++k){int which=(round+k)%4;double value=which==0?measure(d,s):which==1?measure(fp,s):which==2?measure(x,s):measure(lut,s);totals[which]+=value/4;}printf("seed=%u fp64=%.6f fp32=%.6f branch=%.6f lut=%.6f us/token\n",seed,totals[0],totals[1],totals[2],totals[3]);}return 0;}catch(const std::exception&e){printf("FAIL %s\n",e.what());return 1;}}
