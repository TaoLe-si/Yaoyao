...
struct Packed2BitCpuModel{
Config c;std::map<std::string,Vec>w;std::map<std::string,CpuTernary2BitRows>packed;
explicit Packed2BitCpuModel(const CpuModel&src):c(src.c){for(auto&t:schema(c)){if(t.ternary)packed.emplace(t.name,CpuTernary2BitRows(src.w.at(t.name),t.rows,t.cols));else w.emplace(t.name,src.w.at(t.name));}}
#ifdef TAO_CPU_AVX2
bool fast=true;
