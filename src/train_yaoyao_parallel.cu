...
auto prior_identity=identity;identity+="ce-parallel256-block-loss-v1\n";
auto expected=capture(slots,cursor);SlotSnapshot snap;
try{snap=load_slot_file(argv[1],identity,expected,cursor);}catch(const std::runtime_error&e){if(std::string(e.what())!="length"&&std::string(e.what())!="identity")throw;try{snap=load_slot_file(argv[1],prior_identity,expected,cursor);}catch(const std::runtime_error&e2){if(std::string(e2.what())!="length"&&std::string(e2.what())!="identity")throw;snap=load_slot_file(argv[1],old_identity,expected,cursor);}}
