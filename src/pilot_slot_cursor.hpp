...
struct PilotCursor {
std::vector<std::vector<Token>> docs;std::vector<Cursor>slots;std::vector<float> doc_weights;std::vector<std::vector<float>> doc_lp;std::vector<std::vector<float>> doc_lpref;size_t next=0;
