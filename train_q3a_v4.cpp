// train_q3a_v4.cpp
// With weight clipping + decay to prevent Q3 weights from blowing up.
...
const float WD = 0.05f;  // weight decay
...
void clip_weight(float& w, float max_abs) {
    if (w > max_abs) w = max_abs;
    if (w < -max_abs) w = -max_abs;
}
...
void train_task(const std::vector<std::pair<std::vector<int>, int>>& data_in,
    ...
    int best_epoch = 0, best_correct = initial_correct;
    ...
    for (int epoch = 0; epoch < EPOCHS; ++epoch) {
        ...
        // Clip
        clip_weight(w_q3_0[0], W_MAX);
        clip_weight(w_q3_1[0], W_MAX);
        clip_weight(w_q3_2[0], W_MAX);
        clip_weight(w_head, 5.0f);

        if ((epoch + 1) % 10 == 0 || epoch == 0 || epoch == EPOCHS - 1) {
            int c = 0;
            for (auto& pp : data) if (predict(pp.first) == pp.second) c++;
            if (c > best_correct) { best_correct = c; best_epoch = epoch + 1; }
    ...
    std::cout << "\n  Final: " << fc << "/" << data.size()
              << " (" << fc * 100 / data.size() << "%)\n";
    std::cout << "  Best:  " << best_correct << "/" << data.size()
              << " (epoch " << best_epoch << ")\n\n";
}
...
int main() {
    ...
    std::cout << "================================================================\n";
    std::cout << "  Q3 k=3 conv with weight clip + decay\n";
    std::cout << "================================================================\n\n";
