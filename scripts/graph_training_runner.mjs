...
const stem=s.next===60?'build/yaoyao_controlled_step_60':'build/yaoyao_graph_step_'+s.next;
if(s.phase==='training'){
...
await run('build/train_yaoyao_graph.exe',[s.checkpoint],'build/graph_train_'+s.next+'_'+Date.now()+'.log');
...
const log='build/graph_eval_'+s.next+'.log';
