// Step 7: 夭夭 v0.8 - Q1 with real query (context-aware lookup)
// Critical change: each Q1.forward uses previous timestep's output as query
#define D_H 64
#define NL_H 2
#define Q1_B 64
#define Q1_K 16
...
void forward(M& m, const std::vector<int>& inp, int BATCH, int SEQ, int PAD,
             ...
             std::vector<float>& x, std::vector<float>& x_prev,  // x_prev = previous timestep Q1 output (query)
    ...
    int BL=BATCH*SEQ;
// Fixed embedding: always use zero query (no chain propagation).
    // This makes Q1 act as a soft clustered embedding lookup.
    // Each token always gets the same representation (mean of K candidates in its bucket).
    // Note: Aux still stores query pointer for backward compatibility, but query is always zero.
    for(int d=0;d<D;++d) x_prev[d]=0.0f;
    for(int t=0;t<BL;++t){
        int id=inp[t];
        m.q1.forward(id, x_prev.data(), x.data()+t*D, q1_aux[t]);
        // Keep x_prev as zero (fixed embedding, no chain)
        for(int d=0;d<D;++d) x_prev[d]=0.0f;
    }
    for(int l=0;l<NL;++l){
...
int main(){
    ...
    std::printf("Config: BATCH=%d SEQ=%d EPOCHS=%d\n", BATCH, SEQ, EPOCHS);
            ...
            for(int start=0; start+SEQ+1<=(int)inp_full.size(); start+=SEQ){
                ...
                for(int l=NL-1;l>=0;--l){
                    ...
                    if(l>0){for(int n=0;n<BL;++n) for(int d=0;d<D;++d) d_hg[n*D+d]=d_sg[n*D+d]=d_x[n*D+d];}
                        ...
                        // d_x is gradient w.r.t. Q1 output (x buffer)
                        // BPTT through Q1 with REAL query chain:
                        // x[t] = Q1(token[t], x[t-1])
                        // d_x[t] is gradient at x[t].
                        // For t>0, x[t-1] is query to Q1(token[t], x[t-1]).
                        // So gradient flows from d_x[t] back to x[t-1] via Q1's d_query.
                        // But we want to accumulate d_q1_grad for Q1.
                        // Step 1: For each t, call Q1.backward with d_out=d_x[t] to get d_q1_grad and d_query
                        // Step 2: Add d_query to d_x[t-1] (since x[t-1] is query)
                        // Step 3: Forward pass already stored q1_aux[t] with query pointer
                        // (removed stray ref)
                        // Actually we need to accumulate d_x through the chain.
                        // Let me just use d_x as the accumulator: after backward, add d_query to d_x[t-1]
                        for(int t=0;t<BL;++t){
                            std::vector<float> d_q(D,0);
                            m.q1.backward(q1_aux[t], d_x.data()+t*D, m.q1_grad, d_q.data());
                            // Add d_q to d_x[t-1] (or to d_x[t] if it's t=0, but t=0 has no query contribution)
                            // Note: x_prev is the previous timestep's output. If t>0, query at t is x[t-1].
                            // d_x_prev[t-1] += d_q
                            // But wait, x_prev is just one shared variable, not per-timestep.
                            // Let me think: in forward, we iterate t=0..BL-1, with x_prev as a running buffer.
                            // For t=0: query=zero, output=x[0]
                            // For t=1: query=x[0], output=x[1]
                            // So gradient at x[t] comes from:
                            //   - Layer backward (d_x[t] from layer 0)
                            //   - Q1 of token[t+1] (because x[t] is query for token[t+1])
                            // So we need to propagate d_x[t] to BOTH the Q1 trits AND to layer 0.
                            // But we already accumulated the layer 0 part. Now add the Q1 chain.
                            // Actually d_q is dL/d(x[t]) via the Q1 query chain for token[t+1].
                            // Wait no. d_q from backward(q1_aux[t]) is dL/d(query at position t).
                            // Query at position t = x[t-1]. So d_q += d_x[t-1] (via Q1 chain).
                            // So d_x[t-1] += d_q.
                            if(t>0){
                                for(int d=0;d<D;++d) d_x[(t-1)*D+d] += d_q[d];
                            }
                            // For t=0, d_q is gradient w.r.t. the initial zero query, which is unused.
                        }
                    }
                }
    ...
    for(auto& p:qa){
        ...
        for(int step=0;step<25;++step){
            ...
            std::vector<std::pair<float,int>> ranked(V_unit);
            for(int v=0;v<V_unit;++v) ranked[v]={logits[bt*V_unit+v], v};
            std::sort(ranked.begin(), ranked.end(), std::greater<std::pair<float,int>>());
            std::printf("      step %2d top5: ", step);
            for(int r=0;r<std::min(5,V_unit);++r){
                int cp=vocab.decode(ranked[r].second);
                std::printf("%c%.1f ", cp>=0?(char)cp:'?', ranked[r].first);
            }
            std::printf("\n");
            int cp=vocab.decode(best);
            // Stop at end-of-sentence punctuation
            if(cp=='.'||cp=='!'||cp=='?'){
                ids.push_back(best);
                break;
            }
            // Stop if same char repeated 3+ times (e.g., "lll" garbage)
            if(ids.size()>=2){
                int cp_prev=vocab.decode(ids[ids.size()-1]);
                if(cp==cp_prev && cp!=' ' && cp!='	'){
                    int cp_prev2=vocab.decode(ids[ids.size()-2]);
                    if(cp_prev2==cp) break;
                }
            }
            ids.push_back(best);
        }
