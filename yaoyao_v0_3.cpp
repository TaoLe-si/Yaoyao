// 夭夭 Yaoyao v0.3 - Multi-layer Q3 chain + Top-K sampling + LR schedule
// Full BPTT (train Q3 weights), OMP batch parallel, D=128, N_LAYERS=4
...
struct Yaoyao {
    ...
    void forward(const std::vector<int>& input, ForwardCache& cache) const {
        ...
        for (int l = 0; l < N_LAYERS; ++l) {
            ...
            // Next layer's xs = this layer's ys
            if (l + 1 < N_LAYERS) {
    ...
    // Train batch with FULL BPTT through Q3 stack
    float train_batch(const std::vector<std::vector<int>>& inputs,
        ...
        // Per-layer dy gradients (per window, accumulated)
        // dh_l[t+1], ds_l[t+1] - gradients at time t+1 of layer l
        std::vector<std::vector<std::vector<float>>> dy_l_acc(B, 
            std::vector<std::vector<float>>(N_LAYERS * 0, std::vector<float>(D, 0)));  // dummy
        
        // Process each window separately (accumulate gradients)
        for (int b = 0; b < B; ++b) {
            int L = inputs[b].size();
            // dy[l][t][d] = gradient w.r.t. y_l[t][d]
            ...
            // BPTT through layers (last to first), positions last to first
            for (int l = N_LAYERS - 1; l >= 0; --l) {
                ...
                // Propagate dy to previous layer
                if (l > 0) {
                    for (int t = 0; t < L; ++t)
                        for (int d = 0; d < D; ++d)
    ...
    // Top-K sampling: keep top K tokens, sample from them
    std::string generate(const std::string& prompt, int max_new, Vocab& vocab,
        ...
        std::vector<int16_t> s_state(N_LAYERS, 0);  // dummy init
        std::mt19937 rng(123);
        for (int step = 0; step < max_new; ++step) {
            ForwardCache gen_cache;
            forward(out, gen_cache);
            ...
            for (int k = 0; k < top_k; ++k) {
                probs[k] = std::exp(logits[idx[k]] - mx);
                sum += probs[k];
            }
...
int main() {
    g_log.open("D:\\TaoVm\\yaoyao_train.log", std::ios::trunc);
    {
        ...
        std::ostringstream o;
        o << "=== 夭夭 Yaoyao v0.3 log started " << ctime(&now);
        LOG(o.str());
    }
    Vocab vocab;
    ...
    {
        std::ostringstream o;
        o << "\n  Initial (greedy): \"" << gen("Once upon a time", 80, 1, 1.0) << "\"\n";
        o << "  Initial (top-10, T=0.8): \"" << gen("Once upon a time", 80, 10, 0.8) << "\"\n\n";
        LOG(o.str());
    }
    
    int epochs = 15;
    LOG("  Training 15 epochs Adam lr=0.003 -> 0.0005 (with decay)...\n\n");
    auto t0 = std::chrono::steady_clock::now();
    auto last_log = t0;
    for (int epoch = 0; epoch < epochs; ++epoch) {
        // LR decay: linear from 0.003 to 0.0005
        float lr = 0.003f - (0.003f - 0.0005f) * (float)epoch / (float)(epochs - 1);
        ...
        o << "\n  Epoch " << (epoch + 1) << " avg_loss=" << std::fixed << std::setprecision(4)
          << total / n_batches << "\n";
        o << "    greedy: \"" << gen("Once upon a time", 80, 1, 1.0) << "\"\n";
        o << "    top-10 T=0.8: \"" << gen("Once upon a time", 80, 10, 0.8) << "\"\n\n";
        LOG(o.str());
    }
