...
- **跨轮趋势（n=400 全量 held-out，同一评测集）**：
  | 检查点 | loop(pen=0) | bigramF1 | 乱码 | 部署 loop | 部署 F1 |
  |---|---:|---:|---:|---:|---:|
  | 第一轮末（step 3973） | 273/400 (68.3%) | 0.0366 | 14 | 7/400 | 0.0530 |
  | 第二轮末（step 7573） | **230/400 (57.5%)** | **0.0423** | 14 | **2/400** | **0.0543** |
  逐轮**单调改善**：退化复读 −10.8pt、bigramF1 +15.6%，且训练 NLL 持续下降。结论：模型随学习减少重复、提高泛化，**尚未触顶**，值得继续轮次巩固后再混入新语料。注意 n=200 时该差异被噪声掩盖（曾据此误判为"无改善"），跨轮结论必须用 n=400。
...
2. **泛化差距在收敛**：见过 : 没见过 的 F1 差距从第一轮 +18% 收窄到 +9%，第三轮约 +0.7%。差距缩小意味着模型在学可迁移的模式，而不是记住样本。
3. **held-out F1 单调上升**：0.0365 → 0.0414 → 0.0425。
