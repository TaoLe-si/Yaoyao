...
- **当前（第二轮收口 step 7573，第三轮进行中）**：真实能力 loop(pen=0)=62.5%、bigramF1=0.0408；部署解码（乘法全回复）loop=0/200、bigramF1=0.0547。
- **第二轮 vs 第一轮（同 shard 8、同 held-out）**：loop(pen=0) 62.0%→62.5%、bigramF1 0.0403→0.0408 —— **无改善**；而 shard 8 训练 NLL 3.42→2.51，判定为开始记忆化。结论：同数据边际收益已转向数据规模，下一步进入混合语料阶段。
