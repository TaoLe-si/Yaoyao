...
- **数据分片不均（实测发现，影响一切跨轮结论）**：alpaca 分片 0–7 各 6000 docs / 约 970k tokens，而**分片 8 仅 418 docs / 69k tokens**（7%）。训练器按固定步数循环读取，导致分片 8 在 400 步内被**反复遍历约 39 遍**（其他片约 3 遍）。因此"每轮末（分片 8 末）"的检查点是被小片重度过拟合污染的状态，不能用于判定轮次趋势。阶段二语料同样有此问题（code 分片 3.2MB vs 152MB；wiki 末片 28.4MB vs 41MB），需重建等量分片。

- **跨轮趋势（n=400 全量 held-out）**：下表左为**干净边界**（分片 7 末，进入小片之前），右为**污染边界**（分片 8 末）。两者同形但水平不同。

  | 轮次 | 干净 loop | 干净 F1 | 干净精确率 | 干净自重复4 | 污染 loop | 污染 F1 | 污染精确率 | 污染自重复4 |
  |---|---:|---:|---:|---:|---:|---:|---:|---:|
  | 第一轮 | 285/400 | 0.0436 | 0.181 | 0.589 | 273/400 | 0.0366 | 0.152 | 0.522 |
  | 第二轮 | 274/400 | **0.0463** | 0.179 | **0.533** | 230/400 | **0.0423** | 0.156 | **0.444** |
  | 第三轮 | 278/400 | 0.0406 | **0.167** | 0.554 | 198/400 | 0.0372 | 0.131 | 0.394 |

  **修正后的结论（两处修正）**：
  1. "重复率随学习单调下降"**不成立** —— 那是小片污染的假象。干净边界上自重复 4-gram 为 0.589 → 0.533 → **0.554**（第二轮最优，第三轮回升）。
  2. "第三轮相关性退步"**成立且一致** —— 精确率在干净边界 0.181 → 0.179 → **0.167**（−7.6%），在污染边界 0.152 → 0.156 → **0.131**，两个测点同形，说明是真实信号而非分片假象。
  3. 目前**最优轮次是第二轮**：它在两个测点上都取得最好 F1，干净边界上重复度也最低。
...
- **未出现**：数值计算、知识问答、内容组合。退化复读(pen=0) 已从 68.3% 降至 49.5%，但**精确率未同步提高**（第三轮反降 16%），说明减少重复≠提高内容质量。
