...
__global__ void fixed_context(float* z,const float* w,const int* ids,const int8_t* g,int rows){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=rows*V)return;int n=i/V,v=i%V;float x=z[i];for(int k=1;k<16&&k<=n%SEQ;++k){int q=g[ids[n]*15+k-1];if(q)x+=float(q)/32*w[ids[n-k]*V+v];}z[i]=x;}
int main(int argc, char** argv) {
    if (argc < 4) { printf("Usage:\n  %s model.bin tokens.txt output.bin [n_win=100] [epochs=1] [lr=0.005]\n  %s model.bin --fd-check tokens.txt\n", argv[0], argv[0]); return 1; }
    if (argc != 11 || std::ifstream(argv[3]).good()) return 1;
    Model m;
    if (!load_model(argv[1], m)) { printf("load failed\n"); return 1; }
    printf("[V21-Phase5-GPU] Loaded model step=%d\n", m.step);
    std::vector<int8_t> reader(17 * D, 0);
    std::fill(reader.begin(), reader.begin() + D, 1);
    if (std::string(argv[9]) != "identity") {
        std::ifstream input(argv[9], std::ios::binary | std::ios::ate);
        if (!input || input.tellg() != std::streamoff(128 + 17 * D + 4)) return 1;
        input.seekg(0);
        std::vector<unsigned char> data(128 + 17 * D + 4);
        if (!input.read((char *)data.data(), data.size())) return 1;
        uint32_t crc = 0;
        for (int j = 0; j < 4; ++j) crc |= uint32_t(data[data.size()-4+j]) << (8*j);
        if (reader_crc32(data.data(), data.size()-4) != crc) return 1;
        if (data[0]!='R' || data[1]!='T' || data[2]!='S' || data[3]!='1') return 1;
        for (size_t j=0;j<reader.size();++j) {
            if(data[128+j]>2)return 1;
            reader[j]=int8_t(data[128+j])-1;
        }
    }
    std::ifstream gf(argv[10],std::ios::binary|std::ios::ate);if(!gf||gf.tellg()!=std::streamoff(15480))return 3;gf.seekg(0);
    std::vector<unsigned char> p(15480);if(!gf.read((char*)p.data(),p.size()))return 3;
    auto get=[&](int o){uint32_t x=0;for(int j=0;j<4;++j)x|=uint32_t(p[o+j])<<(j*8);return x;};
    if(get(0)!=0x31474354u||get(15476)!=reader_crc32(p.data(),15476))return 3;
    std::vector<int8_t> g(V*15);for(int i=0;i<V*15;++i){int q=int(p[116+i])-8;if(q!=0&&abs(q)!=1&&abs(q)!=2&&abs(q)!=4&&abs(q)!=8)return 3;g[i]=q;}
    for(int id=0;id<V;++id){int count=0,sum=0;for(int k=0;k<15;++k){int q=g[id*15+k];count+=q!=0;sum+=abs(q);}if(count>2||sum>8)return 3;}
    int8_t* dc;cudaCheck(cudaMalloc(&dc,g.size()));cudaCheck(cudaMemcpy(dc,g.data(),g.size(),cudaMemcpyHostToDevice));
    auto starting_step = m.step;
    ...
    int *d_inp, *d_tgt;
    int8_t *d_reader; float *d_reader_mix;
    cudaCheck(cudaMalloc(&d_reader, reader.size()));
    cudaCheck(cudaMemcpy(d_reader, reader.data(), reader.size(), cudaMemcpyHostToDevice));
    cudaCheck(cudaMalloc(&d_reader_mix, BL*D*4));
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        std::mt19937 sampler(epoch * 1234u + 5678u + uint32_t(starting_step));
        ...
        for (int w = 0; w < n_win; ++w) {
            ...
            for (int batch = 0; batch < BATCH; ++batch) {
                int start = std::uniform_int_distribution<int>(0, int(tokens.size())-SEQ-1)(sampler);
                for (int t=0;t<SEQ;++t) {
                    inpBL[batch*SEQ+t]=tokens[start+t];
                    tgtBL[batch*SEQ+t]=tokens[start+t+1];
                }
            }
            ...
            kernelCheck();
            inverse_reader_mix<<<(BL*D+255)/256,256,0,stream>>>(d_trit_f,d_reader,d_reader_mix,BL,SEQ,D,16);
            kernelCheck();
            concat_kernel<<<BL, 1, 0, stream>>>(d_state, d_reader_mix, d_hash_f, BL);
            kernelCheck();
            ...
            // Softmax
            fixed_context<<<(BL*V+255)/256,256,0,stream>>>(d_logits,d_Wbi,d_inp,dc,BL);kernelCheck();
