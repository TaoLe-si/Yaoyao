...
void forward(M& m, const std::vector<int>& inp, int BATCH, int SEQ, int PAD,
    ...
    for(int l=0;l<NL;++l){
        ...
        // RESIDUAL: x_new = x_in + (h_bar + s_bar)
        for(int n=0;n<BL;++n) for(int d=0;d<D;++d) x[n*D+d]=xs[l*BL*D+n*D+d]+hg[n*D+d]+sg[n*D+d];
    }
...
int main(int argc, char** argv){
    ...
    std::printf("Model: V=%d D=%d NL=%d Q1[B=%d,K=%d] Q3_K=%d RESIDUAL=YES NL=%d\n", V_unit, D, NL, Q1_B, Q1_K, Q3_K, NL);
    ...
    std::vector<float> d_hg_in(BL*D); // RESIDUAL: saved input gradient (for residual path)
    ...
    for(int v=0;v<V_unit;++v) for(int d=0;d<D;++d){float s1=0,s2=0; for(int n=0;n<BL;++n){s1+=d_logits[n*V_unit+v]*hg[n*D+d]; s2+=d_logits[n*V_unit+v]*sg[n*D+d];} d_Wh_g[v*D+d]=s1; d_Ws_g[v*D+d]=s2;}
            ...
            for(int l=NL-1;l>=0;--l){
                ...
                // RESIDUAL: save input gradient (= output of layer l-1) for residual path
                std::memcpy(d_hg_in.data(), d_hg.data(), BL*D*sizeof(float));
                ...
                if(l>0){
                    // RESIDUAL: d_hg_new = d_x (channels path) + d_hg_in (identity path)
                    for(int n=0;n<BL;++n) for(int d=0;d<D;++d){
                        float res = d_x[n*D+d] + d_hg_in[n*D+d];
                        d_hg[n*D+d] = d_sg[n*D+d] = res;
                    }
                }
                else{
                    for(int t=0;t<BL;++t){
    ...
    std::string gen=vocab.decode(ids);
        ...
        m.save("D:\\TaoVm\\yaoyao_v14_nl4_fresh.bin");
return 0;
}
